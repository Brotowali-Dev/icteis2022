...
import { LOAD_ABSTRACT } from "@/graphql/queries/index";
import Loading from "@/components/Loading";

const AbstractGuideline = () => {
  const { loading, data, error } = useQuery(LOAD_ABSTRACT);
  if (loading) return <Loading />;
  if (error) return <h1>Error</h1>;
  const datas = data.abstractGuidelines[0];
      ...
      <div className="cardContainer">
        <div className="card">
          <div className="cardContent">
            ...
            <div className="cardDescription">{datas.downloadGuideline}</div>
